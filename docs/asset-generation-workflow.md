...
3. 卡面定稿后，才独立生成该角色的聊天表情。表情生成应同时参考定稿卡面和原三角色的 emote，避免从卡面机械裁切，也避免表情画风漂移。
4. 卡面和表情分别审核、导出；源文件进入 `assets/source/`，公开成品进入 `durian-web/public/assets/`。

## 2. 工具与参数

使用 `gpt-image-2`，质量统一为 `high`：
...
`tools/generate-assets.mjs` 支持重复传入多个 `--input`。输出参数是相对于 `assets/` 的路径：

```bash
IMAGE_MODEL=gpt-image-2 IMAGE_SIZE=1024x1536 IMAGE_QUALITY=high \
